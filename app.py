...
<style>
    ...
    /* Main background - Deep Purple Vegas */
    .stApp {
        background-color: #2d0a4e;
        background-image: url("data:image/svg+xml,%3Csvg width='40' height='40' viewBox='0 0 40 40' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='%231a0630' fill-opacity='0.5'%3E%3Cpath d='M0 0h20v20H0V0zm20 20h20v20H20V20z'/%3E%3C/g%3E%3C/svg%3E");
    }
    ...
    .stButton > button {
        ...
        border: 3px outset #ff8c00 !important;
    ...
    .stButton > button:active {
        border: 3px inset #ff8c00 !important;
    ...
    .stSelectbox > div > div {
        ...
        border: 2px inset #ff1493 !important;
    ...
    [data-testid="stSidebar"] {
        background-color: #1a0630;
        border-right: 3px outset #ffd700;
    }
    ...
    /* Gold divider (was red) */
    .red-divider {
        height: 5px;
        background: linear-gradient(90deg, #ffd700 0%, #ff1493 50%, #ffd700 100%);
        margin: 10px 0;
    }
...
def calculate_glock_index(df):
    """Calculate profit from tailing every lock at $10, using actual odds."""
    settled = df[df['result'].isin(['Win', 'Loss', 'Push'])]
    if settled.empty:
        return 0.0
    profit = 0.0
    for _, row in settled.iterrows():
        ...
        if row['result'] == 'Win':
            # Garrett won, so tailing wins
            if odds < 0:
                # Negative odds: bet $10 to win 10 * (100 / abs(odds))
                profit += 10 * (100 / abs(odds))
            else:
                # Positive odds: bet $10 to win 10 * (odds / 100)
                profit += 10 * (odds / 100)
        elif row['result'] == 'Loss':
            # Garrett lost, so tailing loses the $10 bet
            profit -= 10.00
    return profit
...
glock_index = calculate_glock_index(df)
...
with col4:
    glock_class = "win" if glock_index > 0 else "lose"
    st.markdown(f"""
    <div class="stat-box {glock_class}">
        <h2>${glock_index:+.2f}</h2>
        <p>G LOCK INDEX</p>
        <small>P/L if you tailed every G Lock ($10/bet)</small>
    </div>
    """, unsafe_allow_html=True)
...
with col5:
    ...
    <div class="stat-box {fade_class}">
        <h2>${fade_index:+.2f}</h2>
        <p>FADE INDEX</p>
        <small>P/L if you faded every G Lock ($10/bet)</small>
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    if active_locks.empty:
        ...
        for _, lock in active_locks.iterrows():
            # Get odds, default to N/A if not set
            odds_display = lock.get('odds', 'N/A')
            if pd.isna(odds_display) or odds_display == '':
                odds_display = 'N/A'
            ...
            <div class="lock-entry">
                ...
                <strong>📈 ODDS:</strong> {odds_display}<br>
...
with tab5:
    ...
    # Determine colors based on indices (Vegas neon green for wins)
    glock_bg = "#39ff14" if glock_index > 0 else "#ff0000"
    glock_text = "#000000" if glock_index > 0 else "#ffffff"
    fade_bg = "#39ff14" if fade_index > 0 else "#ff0000"
    fade_text = "#000000" if fade_index > 0 else "#ffffff"
    
    # Generate the share card HTML with Vegas color scheme
    st.markdown(f'''<div style="background: linear-gradient(135deg, #2d0a4e 0%, #1a0630 100%); border: 4px outset #ffd700; padding: 20px; max-width: 400px; margin: 20px auto; box-shadow: 5px 5px 0px #000000;">
<div style="background: linear-gradient(180deg, #ffd700 0%, #ff8c00 100%); border: 3px outset #ffd700; padding: 10px; text-align: center; margin-bottom: 15px;">
<span style="font-family: DotGothic16, sans-serif; font-size: 1.5rem; color: #2d0a4e; text-shadow: 1px 1px #ff8c00;">🔒 LOCKHUB 🔒</span>
</div>
<div style="text-align: center; margin-bottom: 15px; font-family: VT323, monospace; font-size: 1.8rem; color: #ffd700; background-color: #0d001a; padding: 10px; border: 3px solid #ff1493;">{wins}W - {losses}L - {pushes}P</div>
<div style="background-color: #1a0630; border: 3px outset #ffd700; padding: 15px; margin-bottom: 10px; text-align: center;">
<div style="font-family: VT323, monospace; font-size: 2.5rem; color: #ffd700;">{win_rate:.1f}%</div>
<div style="font-family: VT323, monospace; font-size: 1rem; color: #ffd700;">WIN PERCENTAGE</div>
</div>
<div style="background-color: #1a0630; border: 3px outset #ffd700; padding: 15px; margin-bottom: 10px; text-align: center;">
<div style="font-family: VT323, monospace; font-size: 2.5rem; color: #ffd700;">{streak} 🔥</div>
<div style="font-family: VT323, monospace; font-size: 1rem; color: #ffd700;">WIN STREAK</div>
</div>
<div style="background-color: {glock_bg}; border: 3px outset #ffd700; padding: 15px; margin-bottom: 10px; text-align: center;">
<div style="font-family: VT323, monospace; font-size: 2.5rem; color: {glock_text};">${glock_index:+.2f}</div>
<div style="font-family: VT323, monospace; font-size: 1rem; color: {glock_text};">G LOCK INDEX</div>
<div style="font-family: VT323, monospace; font-size: 0.8rem; color: {glock_text};">(at $10/lock)</div>
</div>
<div style="background-color: {fade_bg}; border: 3px outset #ffd700; padding: 15px; text-align: center;">
...
<div style="font-family: VT323, monospace; font-size: 0.8rem; color: {fade_text};">(at $10/fade)</div>
</div>
</div>''', unsafe_allow_html=True)
    
    st.markdown('''<div style="font-family: VT323, monospace; font-size: 1.1rem; text-align: center; margin: 20px 0; color: #ffd700;">📱 To share: Screenshot this card and send it!</div>''', unsafe_allow_html=True)
...
Win %: {win_rate:.1f}%
Win Streak: {streak} 🔥
G Lock Index: ${glock_index:+.2f} (at $10/lock)
Fade Index: ${fade_index:+.2f} (at $10/fade)"""
...
with tab4:
    ...
    if not st.session_state['glock_authenticated']:
        ...
        <div class="info-box" style="background-color: #ffcccc;">
            <center>
            <span style="color: #990000;">⚠️ <b>RESTRICTED</b> ⚠️</span>
            </center>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="info-box" style="background-color: #ccffcc;">
            <center><span style="color: #006600;">✅ <b>ACCESS GRANTED</b> ✅<br>Welcome back, king! 👑</span></center>
        </div>
        """, unsafe_allow_html=True)
        
        # Subtabs within Entry
        entry_tab1, entry_tab2, entry_tab3, entry_tab4 = st.tabs(["➕ New Lock", "✏️ Edit Lock", "⚖️ Settle", "📝 Edit Settled"])
...
with tab3:
    ...
    st.markdown("""
    <div style="font-family: 'VT323', monospace; font-size: 1.2rem; margin: 15px 0; color: #ffd700;">
    ...
    if amount > 0:
        ...
        with conv_col1:
            ...
            <div class="info-box">
                <center>
                <b style="color: #ffd700;">🏌️ ProV1s</b><br>
                <span style="font-size: 1.8rem; color: #39ff14;">{conversions['prov1_balls']:.1f}</span><br>
                <small style="color: #ff1493;">@ $4.75 each</small>
                </center>
            </div>
            """, unsafe_allow_html=True)
        ...
        with conv_col2:
            ...
            <div class="info-box">
                <center>
                <b style="color: #ffd700;">🍺 Labatt Blues</b><br>
                <span style="font-size: 1.8rem; color: #39ff14;">{conversions['labatt_blues']:.1f}</span><br>
                <small style="color: #ff1493;">@ $0.95 each</small>
                </center>
            </div>
            """, unsafe_allow_html=True)
...
st.markdown("""
<div style="text-align: center; font-family: 'VT323', monospace; font-size: 1rem; color: #ffd700; margin-top: 15px;">
    <p>🔒 LockHub v2.0 🔒</p>
    <p style="color: #ff1493;">© 2025 All Rights Reserved</p>
</div>
""", unsafe_allow_html=True)
